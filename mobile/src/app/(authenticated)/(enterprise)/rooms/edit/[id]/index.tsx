...
const EditRoom = (): React.JSX.Element => {
  ...
  const handleSaveChanges = async( data: NewRoomFormData ): Promise<void> => {
    try {
  ...
  useEffect(() => {
    (async() => {
      try {
        ...
        reset(response);
        setRoomImage(response.image);
        setCustomItems(response.customItems);
      } catch(error:unknown) {
    ...
    <LayoutWrapper>
      ...
      { fetchMessage &&
        <Toast
          ...
          onClose={() => setFetchMessage(null)}
          visible={!!fetchMessage}
        />
      }
      
      <SystemLayout 
      title={`Editar ${watch('roomName')}`} 
      description={`Edite a sala`} 
      layoutType={'ENTERPRISE'}      
      tab='ROOMS'
      goBack={() => router.push('/(authenticated)/(enterprise)/rooms')}
      > 
        <ScrollView contentContainerClassName='gap-5 py-6'>
          <View className={`gap-3 rounded-xl border-2 bg-cyan-50/10 border-medroom-primaryLight p-3 flex-col`}>
            <View className='gap-3'>
              <Text className='font-nunito-bold text-2xl text-center text-medroom-primary'>
                Informações gerais
              </Text>
          ...
          <Button.Default
            filled
            icon={{ name: 'edit', size: { width: 20, height: 20 } }}
            label={isSavingRoom ? 'Salvando...' : 'Salvar alterações'}
            onTouch={handleSubmit(handleSaveChanges)}
          />
        </ScrollView>
      </SystemLayout>
    </LayoutWrapper>
